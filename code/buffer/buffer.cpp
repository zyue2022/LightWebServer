...
/*
 * 返回当前已经被读取了多少字节数据，即readPos_所在的位置
 */
...
/*
 * 将readPos_指针向前移动指定数据段大小(Peek()到end这一段)
 */
 ...
 * 返回当前已经被读取的数据的最后一个位置
 */
char *Buffer::beginRead() { return beginPtr_() + readPos_; }

/*
 * const
 * 返回当前已经被读取的数据的最后一个位置
 */
const char *Buffer::beginRead() const { return beginPtr_() + readPos_; }
 ...
 * 计算得到读缓冲区中能够开始写数据的位置
 */
const char *Buffer::beginWrite() const { return beginPtr_() + writePos_; }
...
/*
 * 使用标准库函数将指定大小数据复制到BeginWrite()指定的缓冲区中
 */
...
void Buffer::makeSpace_(size_t len) {
    if (writableBytes() + prependableBytes() < len) {
        ...
        std::copy(beginRead(), beginWrite(), beginPtr_());
...
ssize_t Buffer::readFd(int fd, int *saveErrno) {
    char         buff[65535];  // 用来存储缓冲区装不完的那部分数据
    struct iovec iov[2];
    /*计算还能写入多少数据*/
    ...
    if (len < 0) {
        ...
        *saveErrno = errno;
        ...
        /*读取的数据比缓冲区大，首先将缓冲区的写入指针指到缓冲区尾部，然后再调用Append方法申请多出来的这一部分空间，并将数据复制过去*/
